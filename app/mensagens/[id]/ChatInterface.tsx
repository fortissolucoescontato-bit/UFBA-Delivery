...
export function ChatInterface({ chatId, currentUser, otherPerson, initialMessages, pixKey }: ChatInterfaceProps) {
        ...
        const channel = supabase
            ...
            .on(
                ...
                (payload: { new: any }) => {
                    ...
                    setMessages((prev) => {
                        // Check if message already exists (either from optimistic update or previous realtime event)
                        // We check by content and sender_id if it's very recent (within 2 seconds) to avoid duplicates from optimistic UI
                        ...
                        if (isDuplicate) {
                            // Replace temp message with real one to get the real ID and timestamp
                            return prev.map(m =>
                                (m.id.startsWith('temp-') && m.content === newMsg.content && m.sender_id === newMsg.sender_id)
                                    ? newMsg
                                    : m
    ...
    const handleSend = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Realtime subscription will eventually replace the temp message
        } catch (error) {
        ...
        <div className="flex flex-col h-screen bg-background max-w-2xl mx-auto border-x">
            ...
            {pixKey && (
                <div className="p-3 bg-primary/5 border-b flex items-center justify-between gap-4">
                    <div className="flex-1 min-w-0">
                        <p className="text-xs font-bold uppercase text-primary tracking-wider">Pagar via PIX</p>
