...
describe('sendMessage action', () => {
    ...
    it('throws error if content is too long', async () => {
        const longContent = 'a'.repeat(2001)
        await expect(sendMessage('chat1', longContent)).rejects.toThrow('Mensagem inválida ou muito longa.')
    })

    it('throws error if rate limit is exceeded', async () => {
        // Mock 10 recent messages
        supabaseMock.order.mockResolvedValueOnce({ 
            data: Array(10).fill({ created_at: new Date().toISOString() }), 
            error: null 
        })
        
        await expect(sendMessage('chat1', 'hello')).rejects.toThrow('Limite de mensagens excedido. Aguarde 1 minuto.')
    })
})
